...
import webtop.component.StateButton;
import java.awt.event.*;

public class StateButtonScripter extends Scripter implements StateButton.Listener
{
	private final StateButton button;
	private final String[] values;
	private final int defaultState;

	public StateButtonScripter(StateButton b,webtop.wsl.client.WSLPlayer player,String id,String param,String[] vals,int defVal) {
		...
		if(values.length!=b.stateCount) {
			super.destroy();
			throw new IllegalArgumentException("Must have a value for each StateButton state.");
		}
	...
	protected void setValue(String value) {
		int i;
		for(i=0;i<values.length && !webtop.util.WTString.equal(value,values[i]);++i);
		if(i==values.length) i=defaultState;

		button.setState(i);
	}

	protected String getValue() {return values[button.getState()];}
	...
	public void stateChanged(StateButton sb,int state) {
		...
		else System.err.println("StateButtonScripter: unexpected stateChanged from "+sb);
	}
}
