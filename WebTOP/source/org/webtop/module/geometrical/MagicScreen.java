...
class MagicScreen extends OpticalElement implements X3DFieldEventListener, NumberBox.Listener,
        StateButton.Listener {
    public static final float MIN_WIDTH = 0.1f, MIN_HEIGHT = 0.1f, MAX_WIDTH = 25, MAX_HEIGHT = 50,
            DEF_WIDTH = 12, DEF_HEIGHT = 14, DEF_TRANS = 0, MIN_POS = -400;
    ...
    private SFFloat set_minPosition;
    ...
    private Image theVirtualImage;

    private float transparency;

    public MagicScreen(Geometrical main, float position) {
        ...
        theRealImage = new Image(main, "ImagePoints3", "ImageColors3");
        theVirtualImage = new Image(main, "ImagePoints4", "ImageColors4");
        ...
        set_minPosition = (SFFloat) getSAI().getInputField(getNode(), "set_minPosition");
        ...
        set_minPosition.setValue(MIN_POS / POSITION_SCALE);
        nodePosition.setValue(getPosition() / POSITION_SCALE);

        rulerToggle.addListener(this);
        //rulerToggleScripter = new StateButtonScripter(rulerToggle,getWSLPlayer(),getName(),"ruler",new String[] {"Off","On"},0);
    ...
    public void clear() {
        theRealImage.clear();
        theVirtualImage.clear();
    }

    public void raytraceReal(RayList rays, int count) {

        /*
           // Temporarily disabled (slow)
           // Don't display rays that miss the screen (but don't clip them either)
        ...
        theVirtualImage.clear();
        theRealImage.process(rays, count);
    }

    public void raytraceVirtual(RayList rays, int count) {
        ...
        theRealImage.clear();
        theVirtualImage.process(rays, count);
    }
    ...
    public void readableFieldChanged(X3DFieldEvent e) {
        ...
        if (mode == SIZE) {
            ...
            DebugPrinter.println("MagicScreen: size -> {" + size[0] + "w," + size[1] + "h}");
            ...
            System.err.println("MagicScreen::callback: unexpected mode " + mode + '!');
            return;
        }
        appletUpdate(mode);
    }
    ...
    public void numChanged(NumberBox src, Number value) {
        ...
        if (src == fbWidth) {
            ...
            System.err.println("MagicScreen: unexpected numChanged event from " + src + '!');
        }
    }
    ...
    public void invalidEntry(NumberBox src, Number value) {
        if (src == fbWidth) {
            ...
            System.err.println("MagicScreen: unexpected invalidEvent from " + src);
        }
    }
    ...
    /*public WSLNode toWSLNode() {
        WSLNode node = new WSLNode("magicscreen");
